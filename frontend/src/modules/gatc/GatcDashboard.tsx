...
import { ClipboardCheck, Calendar, CheckCircle, AlertTriangle, Settings, Loader2 } from 'lucide-react';
...
export default function GatcDashboard() {
  ...
  const stats = [
    { label: 'Pending Tests', value: kpis?.pendingTests ?? 0, icon: ClipboardCheck, color: 'bg-blue-500' },
    { label: "Today's Appointments", value: kpis?.todayAppointments ?? 0, icon: Calendar, color: 'bg-yellow-500' },
    { label: 'Completed', value: kpis?.completedTests ?? 0, icon: CheckCircle, color: 'bg-green-500' },
    { label: 'Failed', value: kpis?.failedTests ?? 0, icon: AlertTriangle, color: 'bg-red-500' },
