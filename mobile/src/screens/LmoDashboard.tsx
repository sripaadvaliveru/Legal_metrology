...
export default function LmoDashboard({ navigation }: any) {
  ...
  const stats = [
    { label: "Today's", value: kpis?.todayInspections ?? 0, icon: 'calendar-outline' as const, color: '#3b82f6' },
    { label: 'Assigned', value: kpis?.assignedInspections ?? 0, icon: 'clipboard-outline' as const, color: '#f59e0b' },
    ...
    { label: 'Overdue', value: kpis?.overdueInspections ?? 0, icon: 'alert-outline' as const, color: '#f97316' },
  ];
  ...
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      ...
      <View style={styles.menuSection}>
        <TouchableOpacity style={styles.menuItem} onPress={() => navigation.navigate('Assignments')}>
          <Ionicons name="clipboard-outline" size={24} color="#1f2937" />
          <Text style={styles.menuLabel}>My Assignments</Text>
        ...
        <TouchableOpacity style={styles.menuItem} onPress={() => navigation.navigate('Certificates')}>
          <Ionicons name="ribbon-outline" size={24} color="#1f2937" />
        ...
        <View style={styles.activitySection}>
          ...
          {kpis.recentActivity.slice(0, 5).map((activity) => (
            <View key={activity.id} style={styles.activityItem}>
              ...
              <View style={[styles.statusDot, { backgroundColor: activity.status === 'SCHEDULED' ? '#f59e0b' : '#10b981' }]} />
            </View>
          ))}
        </View>
      )}
